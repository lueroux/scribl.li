...
import { seedTeamEmailVerification } from '@Scriblli/prisma/seed/teams';
...
test('[TEAMS]: send team email request', async ({ page }) => {
  ...
  await page.getByPlaceholder('eg. Legal').fill('test@test.Scriblli.com');
  await page.getByPlaceholder('example@example.com').click();
  await page.getByPlaceholder('example@example.com').fill('test@test.Scriblli.com');
...
test('[TEAMS]: accept team email request', async ({ page }) => {
  ...
  const teamEmailVerification = await seedTeamEmailVerification({
    email: `team-email-verification--${team.url}@test.Scriblli.com`,
    teamId: team.id,
  });
