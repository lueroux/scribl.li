...
const ISSUER = 'Scriblli Email 2FA';
...
export const generateTwoFactorCredentialsFromEmail = ({
  ...
  if (!Scriblli_ENCRYPTION_KEY) {
    throw new Error('Missing Scriblli_ENCRYPTION_KEY');
  }

  const identity = `email-2fa|v1|email:${email}|id:${envelopeId}`;

  const secret = hmac(sha256, Scriblli_ENCRYPTION_KEY, identity);
