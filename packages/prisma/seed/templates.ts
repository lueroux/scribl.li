...
} from '@Scriblli/lib/constants/direct-templates';
import { incrementTemplateId } from '@Scriblli/lib/server-only/envelope/increment-id';
import { prefixedId } from '@Scriblli/lib/universal/id';
...
export const seedTemplate = async (options: SeedTemplateOptions) => {
  ...
  return await prisma.envelope.create({
    data: {
      ...
      recipients: {
        create: {
          email: 'recipient.1@Scriblli.com',
          name: 'Recipient 1',
