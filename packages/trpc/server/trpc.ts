...
import { AppError, genericErrorCodeToTrpcErrorCodeMap } from '@Scriblli/lib/errors/app-error';
import { getApiTokenByToken } from '@Scriblli/lib/server-only/public-api/get-api-token-by-token';
import type { TrpcApiLog } from '@Scriblli/lib/types/api-logs';
...
export const authenticatedMiddleware = t.middleware(async ({ ctx, next, path }) => {
  ...
  // Taken from `authenticatedMiddleware` in `@Scriblli/api/v1/middleware/authenticated.ts`.
