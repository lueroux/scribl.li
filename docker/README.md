# Docker Setup for Scriblli

The following guide will walk you through setting up Scriblli using Docker. You can choose between a production setup using Docker Compose or a standalone container.
...
This setup includes a PostgreSQL database and the Scriblli application. You will need to provide your own SMTP details via environment variables.

1. Download the Docker Compose file from the Scriblli repository: [compose.yml](https://raw.githubusercontent.com/Scriblli/Scriblli/release/docker/production/compose.yml)
...
4. Set up your signing certificate. You have three options:
   ...
   **Option A: Generate Certificate Inside Container (Recommended)**
   ...
   docker exec -e CERT_PASS="$CERT_PASS" -it Scriblli-production-Scriblli-1 bash -c "
   ...
   # Restart container
   docker-compose restart Scriblli
   ```
   ...
   **Option B: Use Existing Certificate**
   ...
   ```yaml
   volumes:
     - /path/to/your/cert.p12:/opt/Scriblli/cert.p12:ro
   ```
...
This will start the PostgreSQL database and the Scriblli application containers.

6. Access the Scriblli application by visiting `http://localhost:3000` in your web browser.

## Option 2: Standalone Docker Container

If you prefer to host the Scriblli application on your container provider of choice, you can use the pre-built Docker image from DockerHub or GitHub's Package Registry. Note that you will need to provide your own database and SMTP host.

1. Pull the Scriblli Docker image:

```
docker pull Scriblli/Scriblli
```

Or, if using GitHub's Package Registry:

```
docker pull ghcr.io/Scriblli/Scriblli
```
...
docker run -d \
  ...
  -v /path/to/your/cert.p12:/opt/Scriblli/cert.p12:ro \
  Scriblli/Scriblli
```

Replace the placeholders with your actual database and SMTP details.

3. Access the Scriblli application by visiting the URL you provided in the `NEXT_PUBLIC_WEBAPP_URL` environment variable in your web browser.

## Success

You have now successfully set up Scriblli using Docker. You can start organizing and managing your documents efficiently.
...
3. **Verify Docker mount:**
   ```bash
   docker exec -it <container_name> ls -la /opt/Scriblli/cert.p12
   ```
...
```bash
# For Docker Compose
docker-compose logs -f Scriblli
...
Check the status of your Scriblli instance:
...
If you encounter any issues or have further questions, please refer to the official Scriblli documentation or seek assistance from the community.

## Advanced Configuration

The environment variables listed above are a subset of those that are available for configuring Scriblli. For a complete list of environment variables and their descriptions, refer to the table below:

Here's a markdown table documenting all the provided environment variables:

| Variable                                                       | Description                                                                                         |
| -------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| `PORT`                                                         | The port to run the Scriblli application on, defaults to `3000`.                                   |
...
| `NEXT_PRIVATE_SIGNING_LOCAL_FILE_PATH`                         | The path to the key file, default `/opt/Scriblli/cert.p12`.                                        |
