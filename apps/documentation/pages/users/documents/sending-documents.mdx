...
The guide assumes you have a Scriblli account. If you don't, you can create a free account [here](https://scribl.li/free-docs).

### Upload Document

Navigate to the [Scriblli dashboard](https://app.Scriblli.com/documents) and click on the "Upload Document" button located at the top right of the page. Select the documents you want to upload and wait for the upload to complete.
...
![Scriblli dashboard](/document-signing/Scriblli-documents-dashboard.webp)
...
The Scriblli editor allows you to create and configure documents or templates. The editor consists of three sections that guide you through the process of preparing your document.
...
![Editor page in the Scriblli dashboard](/document-signing/editor-overview.webp)
...
![Upload and add recipients page in the Scriblli editor](/document-signing/editor-upload-recipients.webp)
...
Scriblli has 5 roles for recipients with different permissions and actions.
...
Scriblli supports 10 different field types that can be added to the document. Each field type collects various information from the recipients when they sign the document.
...
![The available field types in the Scriblli dashboard](/document-signing/Scriblli-document-fields.webp)
...
![Preview section in the Scriblli editor](/document-signing/Scriblli-editor-preview.webp)

### Email Settings

Before sending the document, you can configure the email settings and customize the subject line, message, and sender name.

![Email settings in the Scriblli dashboard](/document-signing/Scriblli-document-email-settings.webp)

If you leave the email subject and message empty, Scriblli will use the default email template.
...
![The email sent to the recipients with the signing link](/document-signing/Scriblli-sign-email.webp)
...
![How to copy the signing link for each recipient from the Scriblli dashboard](/document-signing/Scriblli-signing-links.webp)

The signing link has the following format:

```bash
https://app.Scriblli.com/sign/12ACP777zxQLO52hjj_vCB
```
...
![General Scriblli editor settings](/document-signing/Scriblli-editor-general-settings.webp)
...
![Email Scriblli editor settings](/document-signing/Scriblli-editor-email-settings.webp)
...
Scriblli enables you to set up access control for your documents. You can require authentication for viewing the document.
...
![Security Scriblli editor settings](/document-signing/Scriblli-editor-security-settings.webp)
