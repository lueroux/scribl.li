...
description: Learn how to create a Scriblli API key and authenticate your API requests.
---

# API Authentication

Scriblli uses API keys for authentication. An API key is a unique token that is generated for each client. The client must provide the key whenever it makes an API call. This way, Scriblli can identify the clients making the requests and authorize their access to the API.
...
![A screenshot of the Scriblli's dashboard that shows the dropdown menu when you click on your user avatar](/public-api-images/Scriblli-user-dropdown-menu.webp)
...
![A screenshot of the Scriblli's user settings page that shows the API Tokens page](/public-api-images/api-tokens-page-Scriblli.webp)
...
![A screenshot of the newly created API token in the Scriblli dashboard](/public-api-images/Scriblli-api-key-blurred.webp)

Once you've created the token, Scriblli will display the key on the screen. Make sure to copy the key and store it securely. You won't be able to see the key again once you refresh/leave the page.
...
```bash
curl --location 'https://app.Scriblli.com/api/v2/envelope/create' \
