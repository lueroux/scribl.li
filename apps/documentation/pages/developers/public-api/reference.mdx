---
title: API Reference
description: Reference documentation for the Scriblli public API.
---
...
<Callout type="info">
  ...
  reference, please see the [API Reference](https://openapi.Scriblli.com/).
</Callout>
...
This is done by doing a the following GET [request](https://openapi.Scriblli.com/reference#tag/envelope/POST/envelope/create)

```sh
curl -X GET "https://app.Scriblli.com/api/v2/envelope/envelope_xxxxxxxxxxxxx" \
...
```sh
curl -X POST "https://app.Scriblli.com/api/v2/envelope/create" \
  -H "Authorization: api_xxxxxxxxxxxxxx" \
...
<Callout type="info">
  ...
  [here](https://openapi.Scriblli.com/reference#tag/envelope/POST/envelope/create)
</Callout>

## Use Template

Scriblli allows you to generate documents from templates. This is useful when you have a standard document format you want to reuse.
...
<Callout type="info">
  ...
  [here](https://openapi.Scriblli.com/reference#tag/envelope/POST/envelope/use)
</Callout>
...
The first step is to retrieve the ID of the template you want to use. This can be done via the API or from the Scriblli UI.
...
To generate a document from the template, you need to make a POST request to the [`/api/v2/envelope/use`](https://openapi.Scriblli.com/reference#tag/envelope/POST/envelope/use) endpoint.
...
```sh
curl -X POST "https://app.Scriblli.com/api/v2/envelope/use" \
  -H "Authorization: api_xxxxxxxxxxxxxx" \
  -H "Content-Type: multipart/form-data" \
  -F 'payload={
...
You can now access the document in your Scriblli account dashboard.
...
The API allows you to add recipients to an envelope via the [`/api/v2/envelope/recipient/create-many`](https://openapi.Scriblli.com/reference#tag/envelope-recipients/POST/envelope/recipient/create-many) endpoint.

The following is an example of a request which creates 2 new recipients on the envelope.

```sh
curl https://app.Scriblli.com/api/v2/envelope/recipient/create-many \
  --request POST \
...
The API allows you to add fields to an envelope via the [`/api/v2/envelope/field/create-many`](https://openapi.Scriblli.com/reference#tag/envelope-fields/POST/envelope/field/create-many) endpoint.
...
```sh
curl https://app.Scriblli.com/api/v2/envelope/field/create-many \
  --request POST \
  --header 'Authorization: api_xxxxxxxxxxxxxx' \
  --header 'Content-Type: application/json' \
  --data '{
