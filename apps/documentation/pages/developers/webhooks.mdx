...
1. **Real-time Data Syncing**: Webhooks provide a way to keep data in sync across different platforms. For example, you can keep your system up-to-date with your Scriblli documents by subscribing to events like document creation or signing.
...
3. **Integrating Third-Party Services**: Webhooks can be used to integrate Scriblli with third-party services. For example, you could use a webhook to send data to a CRM system when a document is signed.

Scriblli supports Webhooks and allows you to subscribe to the following events:
...
![A screenshot of the Scriblli's dashboard that shows the dropdown menu when you click on your user avatar](/webhook-images/Scriblli-main-page.webp)

Then, navigate to the "Webhooks" tab, which takes you to the webhooks main page.

![A screenshot of the Scriblli's team settings page that shows the Webhooks tab and the Create Webhook button](/webhook-images/webhooks-main-page.webp)
...
- Optionally, you can provide a secret key that will be used to sign the payload. This key will be included in the `X-Scriblli-Secret` header of the request.
...
![A screenshot of the Scriblli's user settings page that shows the newly created webhook subscription](/webhook-images/webhooks-page.webp)
...
![A screenshot of the Scriblli's team settings page that shows the webhook logs](/webhook-images/webhook-detail-page.webp)

You can go even further and check the execution details of each call by clicking on a specific webhook call.

![A screenshot of the Scriblli's team settings page that shows the webhook call details](/webhook-images/webhook-run-page.webp)
...
![Scriblli's individual webhook page](/webhook-images/webhook-test-trigger.webp)

Choose the event you want to test and click "Send". You’ll then receive a test payload from Scriblli with sample data.
