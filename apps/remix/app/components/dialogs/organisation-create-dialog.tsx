...
export const OrganisationCreateDialog = ({ trigger, ...props }: OrganisationCreateDialogProps) => {
  ...
  const isIndividualPlan = (priceId: string) => {
    return (
      plansData?.plans[INTERNAL_CLAIM_ID.PRO]?.monthlyPrice?.id === priceId ||
      plansData?.plans[INTERNAL_CLAIM_ID.PRO]?.yearlyPrice?.id === priceId
    );
  };
...
const internalClaimsDescription: {
  ...
  [INTERNAL_CLAIM_ID.PRO]: msg`25 documents, custom branding and more`,
  [INTERNAL_CLAIM_ID.BUSINESS]: msg`1000 documents, advanced features and more`,
...
const BillingPlanForm = ({
  ...
  const dynamicPlans = useMemo(() => {
    return [INTERNAL_CLAIM_ID.PRO, INTERNAL_CLAIM_ID.BUSINESS, INTERNAL_CLAIM_ID.PLATFORM].map(
      (planId) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="mt-4 grid gap-4 text-sm">
        ...
        <Link
          to="https://scribl.li/enterprise-cta"
          target="_blank"
